    ...
    for i in range(len(answers)):
        ...
        for j in range(length):
            ...
            for k in range(n_guesses):
                prob_vec[j].append(guesses_scores[j][k][1])
                if prob_vec[j][k] > max_prob:
                    max_prob = prob_vec[j][k]
                    max_word = guesses_scores[j][k][0]
            if max_word == ans[j]:
                labels.append(1)
            else:
                labels.append(0)
...
class RNNBuzzer(nn.Module):
    ...
    def __init__(self, n_input=10, n_hidden=50, n_output=2, dropout=0.5):
        ...
        self.hidden_to_label = nn.Linear(in_features = self.n_hidden, out_features = self.n_output)
    ...
    def forward(self, X, X_lens):
        ...
        reshape_lstm = output_lstm.contiguous().view((len(output_lstm)*len(output_lstm[0])), len(output_lstm[0][0]))

        #Get logits from the final linear layer
        logits = self.hidden_to_label(reshape_lstm)
